...
public class DestructiveTrait extends AbstractTrait {
    public DestructiveTrait() {
    ...
    public boolean onBlockBroken(ItemStack stack, Level dimensionIn, BlockState state, BlockPos pos, LivingEntity user) {
        if (user.getRandom().nextInt(10) == 0) {
            user.level().explode(user, pos.getX() + 0.5F, pos.getY() + 0.5F, pos.getZ() + 0.5F, 1.5F, Level.ExplosionInteraction.TNT);
        }
